		...
		run.addActionListener(new ActionListener() {
			 public void actionPerformed(ActionEvent e) {
				...
				for(int i=0;i<targetstr.length;i++) {   //문자열 돌려보기
					...
					if(point==0) {
						min=frame[0].getorder();
						for(int j=0;j<framenum;j++) {   //hit에 실패하고 빈공간도 없을때
							if(min>frame[j].getorder()) {
								min=frame[j].getorder();
							}
						}
						for(int j=0;j<framenum;j++) {
							if(frame[j].getorder()==min) {
								frame[j].setstr(targetstr[i]);
								frame[j].setorder(ordernum++);
							    migrate++;
							    time+=2000;
