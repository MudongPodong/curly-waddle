...
public class Optimal extends JFrame {
	...
	JLabel pfstr1=new JLabel("PageFault횟수:");//51877415123519968451ABC15
		...
		run.addActionListener(new ActionListener() {
			 public void actionPerformed(ActionEvent e) {
				...
				for(int i=0;i<targetstr.length;i++) {   //문자열 돌려보기
					...
					if(point==0) {
						ordernum=0;
						for(int j=i+1;j<targetstr.length && ordernum!=framenum-1;j++) {	
							for(int k=0;k<framenum;k++) {
								if(frame[k].getstr().equals(targetstr[j])) {
									frame[k].setorder(1);
									ordernum++;
									break;
								}
						...
						for(int j=0;j<framenum;j++) frame[j].setorder(0);	
					}	
	...
	public static void main(String[] args) {
		new Optimal();     //70120304230321201701     123412512345
	} 
}
